...
#              RESPOSTA A FUNÇÃO EXPONENCIAL: u(t) = e^-t
#********************************************************************************
'''
...
for i in range(len(t)):
    u[i] = mth.exp(-t[i])           #Resposta a exponencial
    #u[i] = 3*t[i]                  #Resposta a rampa

T, Y, X = forced_response(sys, t, u)

#plt.plot(T, Y)                         #Plota somente a saída

grafico = plt.plot(T, Y, T, u)          #Plota 2 curvas, o T está em função de ambas
plt.setp(grafico[0], label='Saída')
plt.setp(grafico[1], label='Entrada exponencial')
plt.legend()
plt.show()